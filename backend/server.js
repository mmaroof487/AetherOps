...
environment = "production"
repo_url = "${(businessConfig.repoUrl || "").replace(/"/g, '\\"')}"
`;
		...
		try {
			...
			if (/VpcLimitExceeded/i.test(rawApplyError)) {
				...
				sendEvent("error", {
					message: "AWS VPC quota reached (VpcLimitExceeded). Destroy older deployments and retry. Suggested deployment IDs: " + recentDeployments.join(", "),
				});
			} else {
...
app.get("/api/metrics/:deploymentId", async (req, res) => {
	try {
		...
		const fallbackPoints = (base = 35) => {
			const now = Date.now();
			let current = base;
			const out = [];
			for (let i = 11; i >= 0; i -= 1) {
				current = Math.max(20, Math.min(80, current + (Math.random() * 14 - 7)));
				out.push({
		...
		const deploymentInfoPath = path.join(deployDir, "deployment.json");
		let deploymentInfo = {};
		if (fs.existsSync(deploymentInfoPath)) {
			deploymentInfo = JSON.parse(fs.readFileSync(deploymentInfoPath, "utf8"));
		}

		const outputs = deploymentInfo.outputs || {};
		const readOutputValue = (key) => {
		...
		const instanceId = readOutputValue("app_instance_id") || readOutputValue("instance_id") || readOutputValue("ec2_instance_id");
		const region = deploymentInfo?.credentials?.region || req.session.awsCredentials.region || "ap-south-1";

		if (!instanceId) {
			res.setHeader("x-metrics-fallback", "true");
			res.setHeader("x-metrics-error", "No instanceId found in deployment outputs");
		...
		const points = (metrics.Datapoints || [])
			.filter((dp) => dp.Timestamp && dp.Average != null)
			.sort((a, b) => new Date(a.Timestamp).getTime() - new Date(b.Timestamp).getTime())
			.map((dp) => ({
				time: new Date(dp.Timestamp).toISOString(),
				value: Number(dp.Average.toFixed(2)),
			}));

		if (points.length === 0) {
			res.setHeader("x-metrics-fallback", "true");
			res.setHeader("x-metrics-error", "No CloudWatch datapoints found for instance in selected time range");
			return res.json(fallbackPoints());
		}

		res.setHeader("x-metrics-fallback", "false");
		return res.json(points);
	} catch (err) {
		console.error("Metrics error:", err.message);
		res.setHeader("x-metrics-fallback", "true");
		res.setHeader("x-metrics-error", err.message || "CloudWatch metrics fetch failed");
		return res.json(
			Array.from({ length: 12 }).map((_, idx) => ({
				time: new Date(Date.now() - (11 - idx) * 5 * 60 * 1000).toISOString(),
				value: Number((30 + Math.random() * 35).toFixed(2)),
			})),
		);
	}
});
